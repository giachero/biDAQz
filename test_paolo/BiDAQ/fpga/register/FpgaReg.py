...
class FpgaReg:
    ...
    def __init__(self, BoardList=None):
        ...
        # Create dictionary
        self.RegDict = RegDictClass.CreateDict(BoardList)
    ...
    def ReadRegister(self, RegName):
        ...
        if RegAdr == -1:
            ...
            # Read register data
            Value = self.FPGAMem.read(RegAdr, 1)
            return Value[0]
    ...
    def WriteRegister(self, RegName, Value):
        ...
        if RegAdr == -1:
            ...
            # Write register
            self.FPGAMem.write(RegAdr, [Value])
            return 0
    ...
    def SetBoardSetting(self, RegName, BitName, Data, Board=None):
        ...
        if Board is None:
            for i in self.BoardList:  # Old command with number of boards: # range(0, self.NBoards):
                Ret = self.WriteBits(RegName + str(i), BitName, Data)
                if Ret < 0:
                    break
        else:
            Ret = self.WriteBits(RegName + str(Board), BitName, Data)
    ...
    def GetBoardSetting(self, RegName, BitName, Board):
        return self.ReadBits(RegName + str(Board), BitName)
